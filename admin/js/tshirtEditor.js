	  	...
	  	$(".img-polaroid").click(function(e){
	        ...
	        var opacity = (function(min, max){ return Math.random() * (max - min) + min; })(0.5, 1);
	  		...
	  		fabric.Image.fromURL(el.src, function(image) {
		          ...
		          image.scale(getRandomNum(0.1, 0.10)).setCoords();
		          image.scale(0.10).setCoords();
		          canvas.add(image);
		        });
	  	});
...
// })
	  ...
	  $("#text-strike").click(function() {		  
		  var activeObject = canvas.getActiveObject();
		  if (activeObject && activeObject.type === 'textbox' ) {
			  activeObject.textDecoration = (activeObject.set('linethrough', true) ) ;
		    canvas.renderAll();
		  }
		});
	  $("#text-underline").click(function() {		  
		  var activeObject = canvas.getActiveObject();
		  if (activeObject && activeObject.type === 'textbox') {
			  activeObject.textDecoration = (activeObject.set('underline', true));
		    canvas.renderAll();
		  }
		});
	...
	$('#text-fontcolor').miniColors({
			change: function(hex, rgb) {
			  var activeObject = canvas.getActiveObject();
		      if (activeObject && activeObject.type === 'textbox') {
		    	  activeObject.setColor(this.value) ;
		    	  canvas.renderAll();
		      }
			},
			open: function(hex, rgb) {
	 ...
	 function onObjectSelected(e) {	 
	    ...
	    if (selectedObject && selectedObject.type === 'textbox') {
	    	//display text editor	    	
	    	$("#texteditor").css('display', 'block');
	    	$("#text-string").val(selectedObject.getSelectedText());
